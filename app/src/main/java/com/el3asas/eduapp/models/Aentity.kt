...
@androidx.room.Entity(tableName = "A")
class Aentity(quot: String?, info: String?, comment: String?, num: Int, status: Boolean) :
    Entity(quot, info, comment, num, status) {
    override var status: Boolean
        get() = super.getStatus()
        set(status) {
            super.status = status
        }
    override var quot: String?
        get() = super.getQuot()
        set(quot) {
            super.setQuot(quot)
        }
