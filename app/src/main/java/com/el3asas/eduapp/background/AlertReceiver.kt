...
class AlertReceiver : BroadcastReceiver() {
    ...
    private fun playAzanWithNotification(context: Context, requestCode: Int) {
        val intent = Intent(context, PlayerService::class.java)
        intent.putExtra("requestCode", requestCode)
        ContextCompat.startForegroundService(context, intent)
    }

    private fun setNextPrayAlarm(context: Context, preferences: SharedPreferences) {
        PrayProperties.Companion.getInctance().setPrayProperaties(context, preferences)
    }
}