...
interface QDao {
    // with rxJava
    @Insert
    fun insertQr(entity: Qentity?): Completable?

    @get:Query("select * from Q")
    val allQr: Observable<List<Entity?>?>?

    @Query("update Q set status =:s where num=:num")
    fun updateQStatusr(num: Int, s: Boolean): Completable?

    @Query("select * from Q where num = :n")
    fun findQr(n: Int): Single<Entity?>?

    @Query("select * from Q where status =:s")
    fun getAllLovedQr(s: Int): Flowable<List<Entity?>?>?
}