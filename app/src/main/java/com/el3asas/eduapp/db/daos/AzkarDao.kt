...
interface AzkarDao {
    @Insert
    fun insertAzkar(azkarEntities: List<AzkarEntity?>?): Completable?

    @get:Query("select *from azkar")
    val allAzkar: Single<List<AzkarEntity?>?>?

    @Query("select *from azkar where category =:category")
    fun getAzkarByCat(category: String?): Single<List<AzkarEntity?>?>?

    @Query("select *from azkar where status =:c")
    fun getFavAzkar(c: Boolean): Single<List<AzkarEntity?>?>?

    @Query("select *from azkar where status =:c and category=:cat")
    fun getFavCategAzkar(cat: String?, c: Boolean): Single<List<AzkarEntity?>?>?

    @Query("update azkar set status =:b where id=:id")
