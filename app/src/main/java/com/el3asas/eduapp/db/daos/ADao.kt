...
interface ADao {
    // with rxJava
    @Insert
    fun insertQr(entity: Aentity?): Completable?

    @get:Query("select * from A")
    val allAr: Observable<List<Entity?>?>?

    @Query("update A set status =:s where num=:num")
    fun updateAStatusr(num: Int, s: Boolean): Completable?

    @Query("select * from A where num = :n")
    fun findAr(n: Int): Single<Entity?>?

    @Query("select * from A where status =:s")
    fun getAllLovedAr(s: Int): Flowable<List<Entity?>?>?
}