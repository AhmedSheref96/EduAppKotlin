...
interface PrayDao {
    @Insert
    fun insertPray(prayEntity: PrayEntity?): Completable?

    @get:Query("select * from PrayEntity")
    val allPrays: Observable<List<PrayEntity?>?>?

    @Update
    fun updatePray(prayEntity: PrayEntity?): Completable?
}